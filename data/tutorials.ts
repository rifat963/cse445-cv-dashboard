  ...
  // ── Object Detection ──────────────────────────────────────────────────────
  {
    ...
    tutorials: [
      ...
      },
      {
        ...
        title: "OD 8A: DETR Transformer Detection",
        description:
      ...
      {
        id: "od-08b",
        slug: "rf-detr-transformer-detection",
        title: "OD 8B: RF-DETR Transformer Detection",
        description:
          "Part B extends transformer-based detection to RF-DETR, emphasizing practical training, inference, evaluation, and comparison with standard DETR-style detectors.",
        duration: "90 min",
        difficulty: "Advanced",
        topics: [
          "RF-DETR model workflow: dataset setup, checkpoint selection, training, and inference",
          "Transformer detector outputs: confidence scores, class labels, and bounding boxes",
          "RF-DETR training configuration for custom object-detection datasets",
          "Evaluation against DETR-style baselines using mAP, precision, recall, and F1",
          "Practical strengths and limits of RF-DETR for classroom and project datasets",
        ],
        tools: ["RF-DETR", "PyTorch", "Roboflow", "pycocotools", "Matplotlib"],
        learningObjectives: [
          "Prepare a custom detection dataset for RF-DETR training and validation",
          "Run RF-DETR inference and inspect predicted boxes, labels, and confidence scores",
          "Compare RF-DETR results with DETR using the same validation images and metrics",
          "Explain where RF-DETR improves the practical workflow over a basic DETR pipeline",
        ],
        keyTakeaways: [
          "RF-DETR keeps the transformer detector idea but makes the workflow more usable for applied projects",
          "A fair comparison requires the same dataset split, input size, metric settings, and confidence thresholds",
          "RF-DETR can be a strong choice when students need transformer-based detection without building DETR training from scratch",
          "Qualitative inspection is still necessary because mAP alone hides localization and class-confusion patterns",
        ],
        exercises: [
          "Train or run RF-DETR on a small custom dataset and save predictions for 10 validation images",
          "Create a side-by-side grid comparing DETR and RF-DETR predictions on the same images",
          "Compute mAP@50, mAP@50-95, precision, recall, and F1 for the RF-DETR run",
          "Write a short comparison explaining where RF-DETR succeeds or fails relative to DETR",
        ],
      },
      {
        id: "od-09a",
        slug: "evaluation-error-analysis-deployment",
        title: "OD 9A: Evaluation, Error Analysis & Deployment with YOLO",
        description:
      ...
      {
        id: "od-09b",
        slug: "evaluation-error-analysis-deployment-rf-detr",
        title: "OD 9B: Evaluation, Error Analysis & Deployment with RF-DETR",
        description:
          "Part B repeats the final evaluation workflow for RF-DETR, focusing on transformer-detector error patterns, metric comparison, qualitative diagnosis, and deployment readiness.",
        duration: "90 min",
        difficulty: "Intermediate",
        topics: [
          "RF-DETR validation outputs: prediction JSON, confidence scores, and class-wise metrics",
          "COCO-style evaluation for transformer-based detector results",
          "YOLO vs. RF-DETR error comparison: missed detections, false positives, and localization errors",
          "Confidence threshold selection for RF-DETR deployment scenarios",
          "Deployment readiness checklist: export format, inference latency, memory use, and reproducibility",
        ],
        tools: ["RF-DETR", "pycocotools", "PyTorch", "Pandas", "Matplotlib"],
        learningObjectives: [
          "Run RF-DETR evaluation and interpret mAP@50, mAP@50-95, precision, recall, and F1",
          "Compare RF-DETR and YOLO failure cases using the same validation images",
          "Plot RF-DETR confidence and F1 curves to select a deployment threshold",
          "Prepare a compact evaluation report with quantitative metrics and qualitative examples",
        ],
        keyTakeaways: [
          "RF-DETR and YOLO often fail differently, so paired error analysis is more useful than a single leaderboard score",
          "Transformer detectors may improve difficult context-heavy examples while still needing careful threshold tuning",
          "Deployment decisions should include latency, memory, preprocessing, and postprocessing, not only mAP",
          "A final report should combine tables, curves, and visual examples to explain model behavior",
        ],
        exercises: [
          "Run RF-DETR on the same validation set used for OD 9A and export prediction results",
          "Create a YOLO vs. RF-DETR metrics table with mAP50, mAP50-95, precision, recall, F1, and latency",
          "Build a failure-case gallery showing examples where RF-DETR wins, YOLO wins, and both fail",
